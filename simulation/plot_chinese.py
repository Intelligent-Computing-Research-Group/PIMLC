...
for key in speedup:
    ...
    coloridx=coloridx+1
plt.semilogx()
plt.xlabel("工作负载并行度 $n$",fontsize=38,labelpad=-1.1)
plt.ylabel("加速比",fontsize=40)
plt.title("不同bank数量下的速度提升", size=36)
plt.legend(fontsize=17,ncol=2,frameon=False)
...
plt.xlabel("工作负载并行度 $n$",fontsize=38,labelpad=-1.1)
plt.ylabel("加速比",fontsize=40)
plt.title("不同基准测试下的速度提升", size=36)
...
plt.title("单内存配置下\n平均空间利用率", size=32)
plt.legend(fontsize=13,ncol=2,frameon=False)
...
plt.savefig('figures/PIM_spatialtemporalutil.pdf')
...
fig=plt.figure(figsize=(30,7.4))

ax=plt.subplot(221)
plt.tick_params(labelsize=21)
plt.xticks(xbar+2*width,benchmarks,position=(0,0.04))
plt.bar(xbar, np.array(asicefficiency), width=width, label="ASIC")
plt.bar(xbar+1*width, np.array(smallsimdsramefficiency), width=width, label="SRAM PIM with schedule in [3]")
plt.bar(xbar+2*width, np.array(smallsramefficiency), width=width, label="SRAM PIM with WRAS")
plt.bar(xbar+3*width, np.array(smallsimdrramefficiency), width=width, label="ReRAM PIM with schedule in [3]")
plt.bar(xbar+4*width, np.array(smallrramefficiency), width=width, label="ReRAM PIM with WRAS")
plt.title("Efficiency of workload parallism n=1024",fontsize=28)
plt.ylabel("Efficiency(TOPS/W)",fontsize=24)
plt.semilogy()
for tick in ax.get_xticklabels():
    tick.set_rotation(14)
plt.subplots_adjust(wspace=0.12,hspace=0.32)

ax=plt.subplot(222)
plt.tick_params(labelsize=21)
plt.xticks(xbar+2*width,benchmarks,position=(0,0.04))
plt.bar(xbar, np.array(asicthroughput), width=width, label="ASIC")
plt.bar(xbar+1*width, np.array(simdsramthroughput), width=width, label="SRAM PIM with schedule in [3]")
plt.bar(xbar+2*width, np.array(sramthroughput), width=width, label="SRAM PIM with WRAS")
plt.bar(xbar+3*width, np.array(simdrramthroughput), width=width, label="ReRAM PIM with schedule in [3]")
plt.bar(xbar+4*width, np.array(rramthroughput), width=width, label="ReRAM PIM with WRAS")
plt.title("Throughput of workload parallism n=1024",fontsize=28)
...
plt.xticks(xbar+2*width,benchmarks,position=(0,0.04))
plt.bar(xbar, np.array(asicefficiency), width=width, label="ASIC")
plt.bar(xbar+1*width, np.array(simdsramefficiency), width=width, label="SRAM PIM with schedule in [3]")
plt.bar(xbar+2*width, np.array(sramefficiency), width=width, label="SRAM PIM with WRAS")
plt.bar(xbar+3*width, np.array(simdrramefficiency), width=width, label="ReRAM PIM with schedule in [3]")
plt.bar(xbar+4*width, np.array(rramefficiency), width=width, label="ReRAM PIM with WRAS")
plt.title("Efficiency of workload parallism n=1048576",fontsize=28)
plt.ylabel("Efficiency(TOPS/W)",fontsize=24)
plt.semilogy()
for tick in ax.get_xticklabels():
    tick.set_rotation(14)
...
plt.bar(xbar+3*width, np.array(simdrramthroughput), width=width, label="ReRAM PIM with schedule in [3]")
plt.bar(xbar+4*width, np.array(rramthroughput), width=width, label="ReRAM PIM with WRAS")
plt.title("Throughput of workload parallism n=1048576",fontsize=28)
plt.ylabel("Throughput(TOPS)",fontsize=24)
plt.semilogy()
for tick in ax.get_xticklabels():
    tick.set_rotation(14)
...
fig.legend(lines, labels,fontsize=21,ncol=6,bbox_to_anchor=(0.5,0.99),loc='upper center',frameon=False)
