...
do
    for j in ${benchmark[@]}
    do
        ./simulate ../benchmark/$j.v $cfg ${#workload[@]} ${workload[@]} >> $1
        echo "$j finish"
    done
done
